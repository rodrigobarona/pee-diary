...
export function generatePDFHTML(
  ...
  const groupedByDate = sortedEntries.reduce(
    (acc, entry) => {
...
<body>
        ...
        ${dayEntries
          .map((entry) => {
            ...
            if (entry.type === 'urination') {
              details = `Volume: ${entry.volume} • Urgency: ${getUrgencyLabel(entry.urgency)}`;
              if (entry.hadLeak) tags += '<span class="tag warning">Had leak</span>';
              if (entry.hadPain) tags += '<span class="tag danger">Had pain</span>';
            } else if (entry.type === 'fluid') {
              details = `${entry.amount}ml ${entry.drinkType}`;
            } else if (entry.type === 'leak') {
              details = `Severity: ${entry.severity} • Urgency: ${getUrgencyLabel(entry.urgency)}`;
            }
            ...
            <div class="entry">
              ...
              <div class="entry-content">
                <div class="entry-header">
                  <span class="entry-type">${entry.type === 'fluid' ? 'Fluid Intake' : entry.type}</span>
                  <span class="entry-time">${format(new Date(entry.timestamp), 'h:mm a')}</span>
                </div>
  ...
  <div class="footer">
    Generated on ${format(new Date(), 'MMMM d, yyyy at h:mm a')} • Eleva Diary by Eleva Care
  </div>
</body>
</html>
  `;
